...
#DIREWOLF_INTERNAL_1_SOUND_CARD=
#DIREWOLF_INTERNAL_1_DUAL_SOUND_CARD=No
#DIREWOLF_INTERNAL_1_INPUT_SOUND_CARD=
#DIREWOLF_INTERNAL_1_OUTPUT_SOUND_CARD=
#
# Direwolf default amixer content values
#DIREWOLF_INTERNAL_1_CAPTURE_SWITCH=on
#DIREWOLF_INTERNAL_1_CAPTURE_VOLUME=80%
#DIREWOLF_INTERNAL_1_MASTER_PLAYBACK_SWITCH=on
#DIREWOLF_INTERNAL_1_MASTER_PLAYBACK_VOLUME=80%

# Uppercase $1 for env variable forming
DW_INST=$(echo "$1" | tr '[:lower:]' '[:upper:]')

# Set sound card default
TMP=DIREWOLF_"$DW_INST"_SOUND_CARD ; DIREWOLF_SOUND_CARD="${!TMP:-}"
...
# Set Input defaults
TMP=DIREWOLF_"$DW_INST"_CAPTURE_SWITCH ; DIREWOLF_CAPTURE_SWITCH="${!TMP:-on}"
TMP=DIREWOLF_"$DW_INST"_CAPTURE_VOLUME ; DIREWOLF_CAPTURE_VOLUME="${!TMP:-80%}"
# Set Output defaults
TMP=DIREWOLF_"$DW_INST"_MASTER_PLAYBACK_SWITCH ; DIREWOLF_MASTER_PLAYBACK_SWITCH="${!TMP:-off}"
TMP=DIREWOLF_"$DW_INST"_MASTER_PLAYBACK_VOLUME ; DIREWOLF_MASTER_PLAYBACK_VOLUME="${!TMP:-0%}"
...
echo -e "DIREWOLF_CAPTURE_SWITCH=$DIREWOLF_CAPTURE_SWITCH"
...
echo -e "DIREWOLF_MASTER_PLAYBACK_SWITCH=$DIREWOLF_MASTER_PLAYBACK_SWITCH"
echo -e "DIREWOLF_MASTER_PLAYBACK_VOLUME=$DIREWOLF_MASTER_PLAYBACK_VOLUME"
...
amixer -c "$DIREWOLF_INPUT_SOUND_CARD" cset name='Capture Switch' "$DIREWOLF_CAPTURE_SWITCH"
amixer -c "$DIREWOLF_INPUT_SOUND_CARD" cset name='Capture Volume' "$DIREWOLF_CAPTURE_VOLUME"

echo -e ""
echo -e "${LIGHT_BLUE}Set output parameters:${NC}"
amixer -c "$DIREWOLF_OUTPUT_SOUND_CARD" cset name='Master Playback Switch' "$DIREWOLF_MASTER_PLAYBACK_SWITCH"
amixer -c "$DIREWOLF_OUTPUT_SOUND_CARD" cset name='Master Playback Volume' "$DIREWOLF_MASTER_PLAYBACK_VOLUME"
